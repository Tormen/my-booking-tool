...
class AtomicWriteDirFsyncIntegrationTest(unittest.TestCase):
    ...
    right-path, closes after use, missing dir is best-effort), which is
    also where secure_data_path's own unit tests now live (moved from here
    2026-07-10, when _atomic_write started delegating to the shared
    atomic_write_text() instead of duplicating the mechanics). This class
    is just the integration point: a real _LockedCsv write must call it,
    ...
    def test_atomic_write_fsyncs_the_target_directory(self):
        ...
        # _atomic_write now delegates to app.atomic_io.atomic_write_text, so
        # that's where os.replace/fsync_dir actually get called from.
...
class LockedCsvWritePermissionsTest(unittest.TestCase):
    ...
    still works. See tests/test_atomic_io.py's SecureDataPathTest for the
    underlying helper's own unit tests."""
